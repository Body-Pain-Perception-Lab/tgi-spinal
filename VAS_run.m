...
if vars.control.devFlag == 0
    cbl = str2double(vars.filename.ID(2));
else
    cbl = 1;
end

% for counterbalance procedure 5-8, when control is first, change to 1-4 to match procedure file
if cbl > 4
    order = cbl-4; %recoding back to 1-4
else
    order = cbl;
end
% extract trials associated with specific counterbalancing procedure (1-4)
