...
ggplot(data = vas_h1, aes(colour = condition, group = ID)) +
  ...
  geom_point(data = vas_h1 %>% 
               ...
               aes(xj, VAS), position = position_nudge(.1)) +
  scale_colour_manual(values = c(blue[6], blue[3])) +
  scale_x_continuous(breaks=c(1,2,3), labels=c("cold", "warm", "burn")) +
  labs(title = 'Non TGI', y = 'Visual Analogue Scale 0 - 100', 
       x = 'Sensation') +
...
vas_h1_diff <- vas_h1 %>% 
  pivot_wider(names_from = manipulation, values_from = VAS) %>% 
  mutate(difference = TGI - CNT)
