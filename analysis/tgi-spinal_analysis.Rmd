...
if (!require("pacman")) install.packages("pacman")
  pacman::p_load(boot, broom, broom.mixed, brms, car, caret, corrplot, DHARMa, gamlss,
                 ...
                 modelsummary, polycor, RColorBrewer, rcompanion, reshape2, Rmisc,
...
# Same plot, no zeros
```{r h2nozero}
# First, remove zeros from trials to better represent analysis
vas_nozero <- all_vas %>% 
  filter(VAS > 0)

vas_meds1 <- aggregate(VAS ~ quality*manipulation*condition*cold_probe*ID, 
                      median, data = vas_nozero)

# Change name of manipulation
vas_meds1$manipulation <- factor(vas_meds1$manipulation, labels = c('Non-TGI', 'TGI'))
h2_sum1 <- summarySEwithin(data = vas_meds1, measurevar = 'VAS', 
                          withinvars = c('quality','manipulation','cold_probe'))

# organise data for plotting
vas_meds1$cold_probe <- factor(vas_meds1$cold_probe, levels = 
                                c('proximal', 'distal', 'rostral', 'caudal'))
h2_sum1$cold_probe <- factor(h2_sum1$cold_probe, levels = 
                                c('proximal', 'distal', 'rostral', 'caudal'))
# recoding cold probe location so can jitter
vas_meds1$cold_code <- factor(vas_meds1$cold_probe, labels = 
                                c(1,2,3,4))
h2_sum1$cold_code <- factor(h2_sum1$cold_probe, labels = 
                                c(1,2,3,4))
# creating jitter
vas_meds1$xj <- jitter(as.numeric(vas_meds1$cold_code), amount = .05)
...
ggplot(data = vas_meds1 %>% 
  ...
  geom_half_violin(
    ...
    position = position_nudge(x = nudge1), side = "l", alpha = .4) +
  # summary stats
  geom_point(data = h2_sum1 %>% 
               filter(quality == 'cold'), 
             aes(x = as.numeric(cold_code), y = VAS),
             shape = 21, size = 2, colour = 'grey20', fill = 'grey30',
             position = position_nudge(nudge1)) +
  geom_errorbar(data = h2_sum1 %>% 
               filter(quality == 'cold'), 
               aes(x = as.numeric(cold_code), ymin = VAS-ci, ymax = VAS+ci),
               colour = 'grey20', width = .05, size = .7,
               position = position_nudge(nudge1)) +
  geom_line(data = h2_sum1 %>% 
              filter(quality == 'cold'), 
              aes(x = as.numeric(cold_code), y = VAS), 
              colour = 'grey20', size = .7, alpha = .6,
              position = position_nudge(nudge1)) +
...
# arrange entire plot
h21_plot <- ggarrange(h2_cold_plot, h2_warm_plot, h2_burn_plot,
                     nrow = 2, ncol = 2)
h21_plot

ggsave('h2_nozero_plot.png', h21_plot, device = NULL, path = anaPath,
       width = 10, height = 6.5, dpi = 600)
```
...
ggplot(data = vas_h2_diff %>% 
  ...
  geom_errorbar(data = h2_diff_sum %>% 
               ...
               width = .1, size = .7, colour = 'grey15') +
  scale_colour_manual(values = c(blue[5], oran[5], purp[5])) +
  labs(title = 'Hypothesis 2 Cold & Warm Afferents: Within',
...
ggplot(data = vas_h2_diff %>% 
  ...
  geom_errorbar(data = h2_diff_sum %>% 
               ...
               width = .1, size = .7, colour = 'grey15') +
  scale_colour_manual(values = c(blue[5], oran[5], purp[5])) +
  labs(title = 'Hypothesis 2 Cold & Warm Afferents: Across',
...
model.cold1 = gamlss(VAS ~ manipulation * condition * cold_cond + procedure + trial_n +
                                random(ID),
                    nu.formula = ~ 1+manipulation,
                    tau.formula = ~ 1+manipulation,
                    family = BEINF(mu.link = 'logit', sigma.link = 'logit', nu.link = 'logit', 
                                   tau.link = 'logit'),
                    data = test %>% filter(quality == 'cold'),
                    trace = F) 
mc1 <- summary(model.cold1)
mc1
```
# Warm
```{r warmmod}
...
model.burn1 = gamlss(VAS ~ manipulation * condition * cold_cond + trial_n +
                                random(ID) + random(as.factor(procedure)),
                    nu.formula = ~ 1+manipulation,
                    tau.formula = ~ 1+manipulation,
                    family = BEINF(mu.link = 'logit', sigma.link = 'logit', nu.link = 'logit', 
                                   tau.link = 'logit'),
