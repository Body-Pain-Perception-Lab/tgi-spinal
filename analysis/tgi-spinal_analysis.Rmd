...
cold.eff <- glm(bin~1+manipulation, 
                  family = binomial(link = 'logit'),
                  data = df_long %>% filter(quality == 'cold'))
summary(cold.eff)

# marginal effects from glm
eff <- ggeffects::ggeffect(cold.eff, terms = c('manipulation'))
plot(eff)

eff <- ggeffects::ggeffect(model.cold, terms = c('condition', 'manipulation', 'cold_cond'),
                           type = 'random', ci.lvl = .95)
plot(eff)
```
# Warm
```{r warmmod}
