...
w_dr_tgi_cold <- rnorm(trialN, mean=sample(5:25,1), sd=5)
w_pc_tgi_cold <- rnorm(trialN, mean=sample(5:25,1), sd=5)
a_dr_tgi_cold <- rnorm(trialN, mean=sample(5:50,1), sd=5)
a_pc_tgi_cold <- rnorm(trialN, mean=sample(5:40,1), sd=5)
w_dr_nontgi_cold <- rnorm(trialN, mean=sample(10:60,1), sd=15)
w_pc_nontgi_cold <- rnorm(trialN, mean=sample(10:60,1), sd=15)
...
w_dr_tgi_warm <- rnorm(trialN, mean=sample(10:65,1), sd=15)
w_pc_tgi_warm <- rnorm(trialN, mean=sample(10:70,1), sd=15)
a_dr_tgi_warm <- rnorm(trialN, mean=sample(5:60,1), sd=15)
...
# Burning ratings: simulation
 w_dr_tgi_burn <- rnorm(trialN, mean=sample(10:65,1), sd=15)
 w_pc_tgi_burn <- rnorm(trialN, mean=sample(10:60,1), sd=15)
 a_dr_tgi_burn <- rnorm(trialN, mean=sample(10:70,1), sd=15)
 a_pc_tgi_burn <- rnorm(trialN, mean=sample(5:60,1), sd=15)
...
# running inferential stats on simulated data - to gain idea of effect size

```{r}
library(ez)
library(psychReport)
# isolate just TGI for purposes of determining effect size
df_tgi <- df_subj[df_subj$stimuli == 'tgi' ,]

# burning VAS
burn_anova <- ezANOVA(
  data = df_tgi[df_tgi$quality == 'burn' ,]
  , dv = .(x)
  , wid = .(id)
  , within = .(dermatome, cold_location),
  return_aov = TRUE,
  detailed = TRUE
)
...
# warm VAS
warm_anova <- ezANOVA(
  data = df_tgi[df_tgi$quality == 'warm' ,]
  , dv = .(x)
  , wid = .(id)
  , within = .(dermatome, cold_location),
  return_aov = TRUE,
...
# cold VAS
cold_anova <- ezANOVA(
  data = df_tgi[df_tgi$quality == 'cold' ,]
  , dv = .(x)
  , wid = .(id)
  , within = .(dermatome, cold_location),
  return_aov = TRUE,
...
## t-test
# create seperate data-frames to allow for comparison
within <- df_subj[df_subj$dermatome=='within' ,]
across <- df_subj[df_subj$dermatome=='across' ,]
proxc <- within[within$cold_location== 'prox_caud' ,]
distc <- within[within$cold_location=='dist_rostr' ,]
caudc <- across[across$cold_location=='prox_caud' ,]
rostc <- across[across$cold_location=='dist_rostr' ,]

## H1
names(within)[7] <- 'within'
names(across)[7] <- 'across'
# merge
H1_df <- merge(within, across, by = c('id','cold_location','stimuli','quality'))
# difference
H1_df$diff <- H1_df$within - H1_df$across

H1test <- t.test(H1_df$within, H1_df$across, alternative = 'two.sided', paired = TRUE)
