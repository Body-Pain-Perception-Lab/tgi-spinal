---
title: "TGI Spinal Analysis Code Experiment 1"
author: "A.G. Mitchell"
date: "2022-12-02"
...
# Plot all summary data - for poster :)
...
```{r}
# creating jitter
vas_meds$xj <- jitter(as.numeric(vas_meds$cold_code), amount = .1)
...
ggplot(data = vas_meds %>% 
  ...
  geom_boxjitter(data = vas_meds %>% 
    ...
    position = position_nudge(x = nudge1), width = .5,
      errorbar.length = .3, jitter.shape = 21, jitter.size = 2,
      outlier.shape = NA, errorbar.draw = TRUE, lwd = 1) +
  geom_boxjitter(data = vas_meds %>% 
    filter(quality == 'cold', cold_probe == 'proximal', manipulation == 'TGI'),
    position = position_nudge(x = nudge2), width = .5,
      errorbar.length = .3, jitter.shape = 21, jitter.size = 2,
      outlier.shape = NA, errorbar.draw = TRUE, lwd = 1) +
  geom_boxjitter(data = vas_meds %>% 
  ...
  geom_line(data = h2_sum %>% 
    ...
    position = position_nudge(nudge1), linewidth = 1.5) + 
  geom_line(data = h2_sum %>% 
    filter(quality == 'cold', manipulation == 'TGI'),
    aes(as.numeric(cold_code), VAS), 
    position = position_nudge(nudge2), linewidth = 1.5) +
  ...
  labs(x = NULL, y = 'Cold Rating (0-100)') +
  ylim(-0.5,100) +
  theme_classic() +
  theme(legend.position = 'none',
...
ggsave(here::here("figures", "E1_H2_allDat.png"), plot = h2_plot, 
       device = NULL, width = 7, height = 15, dpi = 600)
```
# Same but box plots
```{r}
# cold 
ggplot(data = vas_meds %>% filter(quality == 'cold'),
        mapping = aes(x = xj, y = VAS, colour = manipulation, fill = manipulation)) +
  geom_boxplot(data = vas_meds %>% 
    filter(quality == 'cold', cold_probe == 'proximal', manipulation == 'Non-TGI'),
    position = position_nudge(x = nudge1), size = 1.2, outlier.size = 2, 
    outlier.alpha = .5) +
  geom_boxplot(data = vas_meds %>% 
    filter(quality == 'cold', cold_probe == 'proximal', manipulation == 'TGI'),
    position = position_nudge(x = nudge2), size = 1.2, outlier.size = 2, 
    outlier.alpha = .5) +
  geom_boxplot(data = vas_meds %>% 
    filter(quality == 'cold', cold_probe == 'distal', manipulation == 'Non-TGI'),
    position = position_nudge(x = nudge1), size = 1.2, outlier.size = 2, 
  ...
  theme(legend.position = 'none',
        axis.text = element_text(size = 16),
        axis.title = element_text(size = 19)) -> cold_box

# warm
ggplot(data = vas_meds %>% filter(quality == 'warm'),
        mapping = aes(x = xj, y = VAS, colour = manipulation, fill = manipulation)) +
  geom_boxplot(data = vas_meds %>% 
    filter(quality == 'warm', cold_probe == 'proximal', manipulation == 'Non-TGI'),
    position = position_nudge(x = nudge1), size = 1.2, outlier.size = 2, 
  ...
  scale_fill_manual(values = c(oran[2], oran[4])) +
  scale_x_continuous(breaks = c(1,2,3,4), labels = c('proximal','distal','rostral','caudal')) +
  labs(x = NULL, y = 'Warm Rating (0-100)') +
  ylim(-0.5,100) +
  theme_classic() +
  theme(legend.position = 'none',
        axis.text = element_text(size = 16),
        axis.title = element_text(size = 19)) -> warm_box

# burn
ggplot(data = vas_meds %>% filter(quality == 'burn'),
        mapping = aes(x = xj, y = VAS, colour = manipulation, fill = manipulation)) +
  geom_boxplot(data = vas_meds %>% 
    filter(quality == 'burn', cold_probe == 'proximal', manipulation == 'Non-TGI'),
    position = position_nudge(x = nudge1), size = 1.2, outlier.size = 2, 
  ...
  scale_fill_manual(values = c(purp[2], purp[4])) +
  scale_x_continuous(breaks = c(1,2,3,4), labels = c('proximal','distal','rostral','caudal')) +
  labs(x = NULL, y = 'Burn Rating (0-100)') +
  ylim(-0.5,100) +
  theme_classic() +
  theme(legend.position = 'none',
        axis.text = element_text(size = 16),
        axis.title = element_text(size = 19)) -> burn_box
```

# Put them all together
```{r}
p_plot <- ggarrange(cold_box, warm_box, burn_box,
                     nrow = 3, ncol = 1)
p_plot

ggsave(here::here("figures", "E1_posterPlot.png"), plot = p_plot, 
       device = NULL, width = 7, height = 15, dpi = 600)
```
...
q = update(model.wcold, data = within %>% filter(quality == 'cold') %>% mutate(manipulation = as.factor(manipulation), manipulation = relevel(manipulation, ref ="TGI")))
summary(q)
...
plot(ggeffect(model.wcold, terms = c("manipulation","cold_probe")))

across %>% filter(quality == 'cold') %>% filter(beta != 0) %>% ggplot(aes(x = manipulation, y = beta, fill = cold_probe))+geom_boxplot(notch = T)
```
...
q = update(model.wwarm, data = within %>% filter(quality == 'warm') %>% mutate(manipulation = as.factor(manipulation), manipulation = relevel(manipulation, ref ="TGI")))
summary(q)
...
q = update(model.wburn, data = within %>% filter(quality == 'burn') %>% mutate(manipulation = as.factor(manipulation), manipulation = relevel(manipulation, ref ="TGI")))
summary(q)
